...
7. Backfill the OpInfo dtype declaration from the probe result.
8. Run the MindSpore opinfo case tests on the same remote machine that produced the probe result.
9. Copy the summaries and logs back locally.
10. Continue the normal op_info analysis loop with those results.
